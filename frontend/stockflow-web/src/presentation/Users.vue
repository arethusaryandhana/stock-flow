  ...
  <div class="page users-page">
    ...
    <main class="settings-content">
        ...
        <section class="surface-card settings-card users-list-card">
          ...
          <div v-else class="table-wrap users-table-wrap"><table class="users-table"><thead><tr><th class="user-identity-column">{{ t('settings.userIdentity') }}</th><th>{{ t('settings.userRole') }}</th><th>{{ t('settings.userStatus') }}</th><th>{{ t('settings.userCreated') }}</th><th v-if="canManage" class="user-actions-column">{{ t('settings.userActions') }}</th></tr></thead><tbody><tr v-for="user in managedUsers.items" :key="user.id"><td><strong>{{ user.fullName }}</strong><small>{{ user.email }}</small></td><td><span class="role-pill" :class="`role-${user.role.toLowerCase()}`">{{ user.role }}</span></td><td><span class="badge" :class="user.isActive ? 'ok' : 'neutral'">{{ user.isActive ? t('settings.active') : t('settings.inactive') }}</span></td><td class="date-cell">{{ formatDate(user.createdAt) }}</td><td v-if="canManage" class="user-actions-column"><button v-if="auth.isAdmin || user.role !== 'Admin'" class="secondary compact-action" type="button" @click="openUserEditor(user)">{{ t('settings.editUser') }}</button><span v-else class="unavailable-action" aria-hidden="true">—</span></td></tr></tbody></table></div>
...
.users-table .user-identity-column { width: 100%; }
.users-table .date-cell { white-space: nowrap; }
.users-table .user-actions-column { width: 1%; padding-right: 20px; padding-left: 20px; text-align: right; white-space: nowrap; }
.users-table-wrap td small { display: block; margin-top: 4px; }
.unavailable-action { color: var(--muted-2); }
