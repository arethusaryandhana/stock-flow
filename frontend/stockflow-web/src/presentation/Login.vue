    ...
    <section class="login-form-side">
      <div class="login-card">
        <template v-if="mode === 'login'">
          ...
          <form class="login-form" autocomplete="on" :aria-busy="busy" @submit.prevent="submitLogin">
            <label class="login-label" for="login-email">{{ t('login.workEmail') }}<input id="login-email" v-model="email" name="username" type="email" autocomplete="username" required :disabled="busy"></label>
            <label class="login-label" for="login-password">{{ t('login.password') }}<div class="password-field"><input id="login-password" v-model="password" name="password" :type="showPassword ? 'text' : 'password'" autocomplete="current-password" required :disabled="busy"><button class="password-toggle" type="button" :disabled="busy" :aria-label="showPassword ? t('login.hidePassword') : t('login.showPassword')" :aria-pressed="showPassword" @click="showPassword = !showPassword"><svg class="eye-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 12s3.75-6 10-6 10 6 10 6-3.75 6-10 6-10-6-10-6Z" /><circle cx="12" cy="12" r="2.75" /><path v-if="!showPassword" d="m3 3 18 18" /></svg></button></div></label>
            <div class="login-options"><label class="checkbox"><input v-model="rememberMe" name="rememberMe" type="checkbox" :disabled="busy"> {{ t('login.rememberMe') }}</label><button class="text-link" type="button" :disabled="busy" @click="showForgot">{{ t('login.forgotPassword') }}</button></div>
            <p v-if="error" class="alert">{{ error }}</p><p v-if="feedback" class="form-feedback">{{ feedback }}</p>
            <button class="primary login-submit" type="submit" :disabled="busy"><span v-if="busy" class="button-spinner" aria-hidden="true" />{{ busy ? t('login.checkingAccess') : t('login.submit') }}</button>
          </form>
