...
List endpoints use server-side pagination. Products, categories, suppliers, customers, stock movements, and stock adjustments accept `page` and `pageSize` (capped at 100), plus their supported search/filter parameters (`search`; products also support `status` and `categoryId`; movements support `type` and `periodDays`). Responses include `items`, `page`, `pageSize`, `totalCount`, and `totalPages`; filtering is applied before `Skip/Take` so the API never loads the complete table for a normal screen request.
