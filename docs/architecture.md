...
List endpoints use server-side pagination. Products, categories, suppliers, customers, purchase orders, sales orders, goods receipts, stock movements, and stock adjustments accept `page` and `pageSize` (capped at 100), plus their supported search/filter parameters (`search`; products also support `status` and `categoryId`; purchase and sales orders also support `status`; movements support `type` and `periodDays`). Responses include `items`, `page`, `pageSize`, `totalCount`, and `totalPages`; filtering is applied before `Skip/Take` so the API never loads the complete table for a normal screen request. Purchase- and sales-order responses also include `statusCounts`, calculated by the API after search filtering and before status filtering, so the UI can render every status tab count without separate requests.
